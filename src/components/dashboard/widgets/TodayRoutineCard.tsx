  ...
  return (
    <div className="block">
      <Card className="relative h-full overflow-hidden p-5 shadow-soft glass">
        <div className="absolute -right-8 -top-8 h-28 w-28 rounded-full bg-gradient-accent opacity-20 blur-2xl" />
        <div className="relative space-y-3">
          <div className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-accent">
                <ListChecks className="h-4 w-4 text-accent-foreground" />
              </div>
              <p className="text-xs font-semibold text-foreground/80">আজকের রুটিন</p>
          </div>
          {loading ? (
            ...
            <>
              <div className="flex items-baseline gap-2">
                ...
                <p className="text-xs text-muted-foreground font-en font-medium">{toBn(done)}/{toBn(eff)}</p>
              </div>
              <div className="h-1.5 overflow-hidden rounded-full bg-muted">
                <div className="h-full bg-gradient-accent transition-spring" style={{ width: `${pct}%` }} />
              </div>
              {skipped > 0 && <p className="text-[11px] text-muted-foreground font-medium">{toBn(skipped)} টি স্কিপ</p>}
            </>
          )}
        </div>
      </Card>

...
export const AIInsights = () => {
  ...
  const generate = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("finance-insights", { body: {} });
      ...
      <div className="relative space-y-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-semibold">AI আর্থিক অন্তর্দৃষ্টি</h3>
              <p className="text-xs text-muted-foreground">গত ৩০ দিনের ব্যয় বিশ্লেষণ</p>
            </div>
          </div>
          <Button
            size="sm"
        ...
        {loading && !insights ? (
          ...
          <div className="rounded-lg border border-dashed border-border/60 bg-muted/30 p-6 text-center">
            <p className="text-sm text-muted-foreground">
              আপনার ব্যয়ের প্যাটার্ন, anomaly ও সঞ্চয় কৌশল সম্পর্কে AI বিশ্লেষণ পেতে বাটনে ক্লিক করুন।
            </p>
          </div>
        )}
      </div>

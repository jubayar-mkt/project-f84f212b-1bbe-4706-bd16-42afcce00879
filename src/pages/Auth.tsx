...
import { Loader2, Mail, Lock, User as UserIcon, RefreshCw, AlertCircle } from "lucide-react";
...
const Auth = () => {
  ...
  const handleGoogle = async () => {
    ...
    setGoogleError(null);
    toast.loading("Google-এ redirect করা হচ্ছে...", { id: "google-auth" });
    try {
      const result = await lovable.auth.signInWithOAuth("google", {
        redirect_uri: `${window.location.origin}/dashboard`,
      });
      if (result.error) {
        const msg = result.error.message || "Google login ব্যর্থ হয়েছে";
        setGoogleError(msg);
        toast.error(msg, { id: "google-auth" });
      ...
      toast.success("স্বাগতম! 🎉", { id: "google-auth" });
      navigate("/dashboard");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "অপ্রত্যাশিত ত্রুটি ঘটেছে";
      setGoogleError(msg);
      toast.error(msg, { id: "google-auth" });
      setGoogleLoading(false);
    }
      ...
      <main className="relative z-10 flex min-h-[calc(100vh-100px)] items-center justify-center p-6">
        <Card className="w-full max-w-md p-8 glass shadow-elevated animate-scale-in">
          ...
          <Button
            ...
            variant="outline"
            className="press w-full mb-3 border-border/80 hover:bg-muted"
          >
            {googleLoading ? (
              <>
              ...
              <>
                ...
                {googleError ? "আবার চেষ্টা করুন" : "Google দিয়ে চালিয়ে যান"}
              </>
            )}
          </Button>

          {googleError && !googleLoading && (
            <div className="mb-4 flex items-start gap-2 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive animate-fade-in">
              <AlertCircle className="mt-0.5 h-3.5 w-3.5 shrink-0" />
              <div className="flex-1 space-y-1.5">
                <p className="font-medium">Google login ব্যর্থ হয়েছে</p>
